...
(async()=>{
 const dir=path.resolve('tmp/tcc_report/qa_20260909_nivel8a');fs.mkdirSync(dir,{recursive:true});
 ...
 assert.equal(structure.chapters,15);assert.equal(new Set(structure.ids).size,structure.ids.length);assert.equal(structure.figures,5);assert.ok(structure.pageWidth<=structure.viewport+1);
