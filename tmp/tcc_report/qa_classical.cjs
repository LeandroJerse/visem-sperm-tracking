...
(async()=>{
 const dir=path.resolve('tmp/tcc_report/qa_20260911_classical'); fs.mkdirSync(dir,{recursive:true});
 ...
 for(const [width,height,label] of [[1440,1000,'desktop'],[390,844,'mobile']]){
  ...
  await page.screenshot({path:path.join(dir,`atlas_comparison_${label}.png`)});
 }
