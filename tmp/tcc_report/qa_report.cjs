...
(async()=>{
 const dir=path.resolve('tmp/tcc_report/qa_20260909');fs.mkdirSync(dir,{recursive:true});
 ...
 assert.equal(structure.chapters,15);assert.equal(new Set(structure.ids).size,structure.ids.length);assert.equal(structure.figures,4);assert.ok(structure.pageWidth<=structure.viewport+1);
 ...
 await page.locator('#resultados figure').last().screenshot({path:path.join(dir,'resultados_fluxo_causal.png'),animations:'disabled'});
 ...
 const nojs=await browser.newContext({javaScriptEnabled:false,viewport:{width:1440,height:1000}});const plain=await nojs.newPage();await plain.goto(pathToFileURL(file).href);assert.ok(await plain.locator('main').isVisible());assert.equal(await plain.locator('main > section').count(),15);await nojs.close();
