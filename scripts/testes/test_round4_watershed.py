"""Plano, execução sintética e relatório completo do round4."""
...
from analise import relatorio_round4_watershed as pdf
...
from scripts.watershed import execucao_round4 as runner
from scripts.watershed import planejamento_round4 as desenho
...
def origens_sinteticas(p, codigo):
    ...
    p2bytes = (desenho.RAIZ / desenho.ROUND3 / "plano.json").read_bytes()
    p1bytes = (desenho.RAIZ / desenho.ROUND3 / "origens/round1_plano.json").read_bytes()
    m2 = {"situacao": "concluida", "avaliacoes_concluidas": 4272, "configuracoes_concluidas": 24,
          "criterios": runner.comum.CRITERIOS, "plano_sha256": sha(p2bytes), "codigo": {"sha256_zip": sha(codigo)},
          "execucoes": [{"configuracao_id": c["referencia_round3"], "pasta": "sintetico/"+c["referencia_round3"]}
                       for c in p["configuracoes"][:6]],
          "saidas_sha256": {f["arquivo"]: f["sha256"] for f in p["fontes_controles"]}}
    mbytes = bytes_json(m2)
    return {"round3_plano.json": p2bytes, "round3_execucao.json": mbytes, "round3_codigo.zip": codigo,
            "round1_plano.json": p1bytes,
            "round2_plano.json": (desenho.RAIZ / desenho.ROUND3 / "origens/round2_plano.json").read_bytes(),
            "estatisticas_round3.json": bytes_json({"manifesto_sha256": sha(mbytes), "situacao": "conferida"})}


class PlanoTest(unittest.TestCase):
    def test_reproducao_referencias_e_12_configuracoes_novas(self):
        p = plano()
        self.assertEqual(p, desenho.construir_plano())
        self.assertEqual(len(p["configuracoes"]), 18)
        self.assertEqual(len(p["fontes_controles"]), 4272)
        self.assertEqual(len({c["parametros_sha256"] for c in p["configuracoes"]}), 18)
        self.assertEqual([c["referencia_round3"] for c in p["configuracoes"][:6]], list(desenho.CONTROLES))
        p2 = carregar_json((desenho.RAIZ / desenho.ROUND3 / "plano.json").read_bytes())
    ...
    def test_recusa_alteracoes_reservados_e_controles_ausentes(self):
        for mudar in (lambda p:p.update(seed=True), lambda p:p["quadros"][0].update(video_id="14"),
                      ...
                      lambda p:p["configuracoes"][6].update(contraste_com="r4c05")):
            p = plano(); mudar(p)
    ...
    def test_dispatch_conferencia_e_recuperacao_pdf(self):
        with patch.object(runner, "executar_argumentos", return_value=0) as executar:
            self.assertEqual(runner.comum.main(["--rodada", "round4", "--conferir"]), 0)
            self.assertTrue(executar.call_args.args[0].conferir)
            self.assertEqual(runner.comum.main(["--rodada", "round4", "--somente-relatorio", "batch"]), 0)
            self.assertEqual(executar.call_args.args[0].somente_relatorio, Path("batch"))


class ExecucaoTest(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="watershed_round4_sintetico_")
        self.addCleanup(self.tmp.cleanup); self.raiz = Path(self.tmp.name).resolve()
        self.saida = self.raiz / "resultados/frame-to-frame/watershed/round4"
    ...
    def test_18_configuracoes_controles_e_reexecucao_sinteticas(self):
        ...
        self.assertEqual(m["situacao"], "concluida"); self.assertEqual(m["avaliacoes_concluidas"], 36)
        ...
        self.assertEqual(len(list(pasta.rglob("segmentacao.json"))), 36)
    ...
    def test_falha_controle_e_interrupcao_preservam_parciais(self):
        ...
        with patch.object(pdf, "gerar_relatorio") as gerar:
            with self.assertRaisesRegex(ValueError, "diverge do round3"): self.rodar()
...
class RelatorioTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.tmp = tempfile.TemporaryDirectory(prefix="pdf_round4_sintetico_"); cls.raiz = Path(cls.tmp.name).resolve()
    ...
    def setUp(self):
        ...
        self.stack.enter_context(patch.object(desenho, "MANIFESTO_SHA256", sha((self.batch/"origens/round3_execucao.json").read_bytes())))

    def test_pdf_completo_e_recuperacao(self):
        d = pdf.carregar(self.batch)
        self.assertEqual(len(d["quadros"]), 3204); self.assertEqual(len(d["videos"]), 216)
    ...
    def test_recusa_batch_incompleto_controle_e_metadados_alterados(self):
        for nome, mudar, erro in (
            ("execucao.json", lambda b:bytes_json({**carregar_json(b),"avaliacoes_concluidas":3203}), "incompleta"),
            ("r4c06/quadros/11_frame_0/predicoes.txt", lambda b:b+b"alterado", "Saída alterada"),
            ("r4c18/quadros/11_frame_0/segmentacao.json", lambda b:b+b"alterado", "Saída alterada")):
