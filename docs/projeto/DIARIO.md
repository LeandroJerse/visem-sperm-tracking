...
A checagem de navegação inicialmente incluiu cópias de README dentro dos
snapshots de auditoria, cujos links relativos preservam a localização original.
O coletor passou a excluir somente os diretórios `before` e
`documentation_before` dentro de `data/derived/project_audits`; documentos
canônicos e os demais READMEs de dados continuam verificados. Após esse ajuste,
o teste de links passou em 1,64 s. Backups e resultados científicos não foram
alterados. O ajuste pertence ao fechamento documental, não ao código das runs.
