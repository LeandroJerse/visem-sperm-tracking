...
Two video sources are supported, mirroring the exploratory detection menu:

* ``--id <N>``    annotated VISEM-Tracking video at
  ``data/sources/visem_tracking/dataset/Train/<N>/<N>.mp4`` with per-frame
  ground truth in ``labels_ftid/`` (matched by the frame number encoded in the
  filename; missing files are unannotated gaps, not empty negative frames).
...
from src.detection.io import GroundTruthFrame, load_gt_for_frame
...
def load_gt(
    gt_dir: Path | None, frame_index: int, w: int, h: int
) -> GroundTruthFrame | None:
    """Return GT while preserving ``annotated`` versus ``unlabeled``.

    ``None`` means that the source is a raw video with no ground truth. For an
    annotated VISEM-Tracking source, a missing label file returns
    ``GroundTruthFrame(annotated=False)`` and must never be scored as empty GT.
    """
    if gt_dir is None:
        return None
    return load_gt_for_frame(gt_dir, frame_index, w, h)
