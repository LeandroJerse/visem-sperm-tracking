...
Official commands and interpretation belong in script/README.md. A benchmark
measures cost; only a complete coarse batch can produce a training shortlist.
"""
...
def _check_budget(plan: dict, mode: str, start: float, monitor: ResourceMonitor,
                  artifact_bytes: int = 0) -> None:
    budget = plan["budget"]
    seconds = float(budget[f"{mode}_soft_wall_seconds"])
...
def _check_benchmark(path: Path, plan: dict, sample: Any) -> dict:
    ...
    if summary.get("completed_candidates") != len(expand_coarse_candidates(plan)):
    ...
    for record in records:
        ...
        if (child.get("status") != "complete" or child_summary.get("mode") != "benchmark"
        ...
        candidate_ids.append(child_summary["configuration_id"])
    ...
    expected_ids = [item["configuration_id"] for item in expand_coarse_candidates(plan)]
    if sorted(candidate_ids) != sorted(expected_ids):
        raise ValueError("Benchmark não cobre exatamente os candidatos planejados.")
    projected = float(summary["projected_coarse_seconds"])
    if not np.isfinite(projected) or projected < 0:
        raise ValueError("Projeção de custo inválida.")
    ceiling = float(plan["budget"]["allow_coarse_after_benchmark_if_projection_seconds_at_most"])
    ...
    return {"path": str(path), "sha256": sha256_file(path), "projected_coarse_seconds": projected}
...
def run_candidate(candidate: dict, sample: Any, plan: dict, mode: str,
    ...
    config.setdefault("provenance", {}).update(
        ...
        sample_hash=sample.sample_hash, batch_id=batch_id, sample_mode=mode,
        interpretation="cost_calibration_do_not_select" if mode == "benchmark" else "training_screening",
    ...
    expected = [(str(vid), int(idx)) for vid in sample.video_ids
                for idx in sample.indices(mode, vid)]
    ...
    try:
        ...
        for video_id, frame, pixels, gt in sample.frames(mode):
            _check_budget(plan, mode, batch_start, batch_monitor)
            started = time.perf_counter()
            detections = detector.detect(pixels)
            detect_elapsed = time.perf_counter() - started
            detector_seconds += detect_elapsed
            if len(detections) > int(plan["budget"]["max_predictions_per_frame"]):
                raise SearchBudgetExceeded(
        ...
        summary = summarize_candidate(
            ...
            expected_frames_per_video={str(vid): len(sample.indices(mode, vid)) for vid in sample.video_ids},
        )
...
def run_batch(plan: dict, sample: Any, mode: str, *, cache_validation_seconds: float,
              benchmark: dict | None = None, output_root: Path | None = None) -> Path:
    if mode not in {"benchmark", "coarse"}:
        raise ValueError("Somente benchmark ou coarse podem executar candidatos.")
    require_clean_repository()
    if mode == "coarse":
        if not benchmark or "path" not in benchmark:
            raise ValueError("Busca grossa exige benchmark completo e compatível.")
        benchmark = _check_benchmark(Path(benchmark["path"]), plan, sample)
    ...
    try:
        ...
        if mode == "benchmark":
            ratio = plan["sampling"]["coarse_frames_per_video"] / plan["sampling"]["benchmark_frames_per_video"]
            batch_summary["projected_coarse_seconds"] = (
                cache_validation_seconds + plan["budget"]["projection_safety_factor"] * ratio * loop_seconds
            )
            ...
            write_json_exclusive(batch.path / "shortlist.json", ranked[:int(plan["selection"]["coarse_shortlist_size"])])
            batch_summary.update(selection_allowed=True, interpretation="training_shortlist_not_promoted")
        batch_artifacts = {
...
def main(argv: list[str] | None = None) -> Path | None:
    ...
    parser.add_argument("--mode", required=True, choices=("prepare", "benchmark", "coarse"))
    ...
    if args.dry_run:
        print(json.dumps({"plan_id": plan["plan_id"], "candidates": len(candidates),
                          ...
                          "sampling": plan["sampling"], "budget": plan["budget"]}, ensure_ascii=False, indent=2))
        return None
    require_clean_repository()
    ...
    if args.mode == "coarse":
        ...
        benchmark = _check_benchmark(resolve_from_repository(args.benchmark_manifest), plan, sample)
